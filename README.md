...
Режим `2D — blueprint связей` — третья presentation-only проекция той же `Aset` и общего `visualModel`. Геометрия адаптирована из `konard/links-visuals` по точному снимку `f377441533e4f10fa94aaa07138b684df88234b1`; upstream опубликован под `Unlicense`.

Одна семантическая связь отображается одной гладкой SVG-формой `start → center → end`. Начало и конец геометрически закреплены на центрах `start(X)` и `end(X)`. Цвета остаются собственным общим контрактом `anum_parser`: `RED → GREEN → BLUE`, а синяя стрелка отмечает конец. Drag, pan, zoom, fit, selection и debugger меняют только представление; сериализованная `Aset` не изменяется. Самопетля остаётся видимой и конечной.

Структурный 2D, blueprint и живая 3D-механика используют одну семантическую модель, но разные renderer-ы; при переключении ресурсы неактивного renderer-а освобождаются.
