...
test("integration: полный postprocess запускается после layoutstop, но не на manual position", async () => {
  ...
  assert.match(source, /const result = optimizeAsetLayoutPositions\(aset, positions\)/);
  ...
  assert.doesNotMatch(positionHandler[1], /optimizeAsetLayoutPositions|postprocessLayout/);
});
